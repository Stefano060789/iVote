...
A fuller, developer-facing action list from the most recent review pass (idempotent webhooks, KYC gating, consent/retention/DSAR, prize-draw gating, tax classification, PCI scope, subprocessor DPAs, monitoring, and geo-gating) is tracked in the project's internal `TODO.md` under "External legal review (2026-09-14) - action tracker", including which items are now code-complete and which still need a business or legal decision rather than more code.
